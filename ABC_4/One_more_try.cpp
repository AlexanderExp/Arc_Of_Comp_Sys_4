...
void clientLeft(Client *client) {
    ...
    backlog += "Client with id " + std::to_string(client->getID()) + " has left\n";
...
void *clientSettled(void *param) {
    ...
    while (!client->isReadyToLeave()) {
        ...
        if (number_of_free_rooms > 0) {
            ...
            backlog += "Client with id " + std::to_string(client->getID()) + " has settled in room number " + std::to_string(number_of_free_rooms+1) + "\n";
            // Разблокируем поток
            pthread_mutex_unlock(&receptionist_sem);
